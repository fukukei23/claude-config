...
#
# 仕組み（2段階state・重複発火抑制）:
#   - ssot-sync-triggered  : 今日の「発火指示済み」フラグ（本スクリプトが先行マーク）
#   - ssot-sync-last-run   : 最終成功日（commit完了日・SKILL.md が更新）
#
...
TRIGGERED="$STATE_DIR/ssot-sync-triggered"
LAST_RUN="$STATE_DIR/ssot-sync-last-run"
TODAY=$(date +%Y-%m-%d)
...
本日（${TODAY}）の ssot-sync auto を未実行なら実行してください（無人モード・高重要度のみ自動修正）。
前回の成功日: ${LAST:-（未実行）}

実行時の厳守事項:
- auto モード: フェーズ2承認スキップ・重要度「高」のみ自動修正・中低は10_DAILY記録
- 安全装置: 削除系（リポ消失等）はスキップ・5ファイル超/100行超でcommit中止
- 完了後 必ず: date +%Y-%m-%d > ~/.claude/state/ssot-sync-last-run （実行成功日の記録・triggeredとは別管理）
EOF
