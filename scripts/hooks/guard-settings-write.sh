...
#
# 仕組み:
# - PreToolUse(Bash): コマンド文字列に「settings.json書込+ネットワーク送信」チェインがあれば exit2
...
python3 - "$CORE" "$SETTINGS" "$SETTINGS_LOCAL" "$SNAPSHOT_DIR" "$LOG_PATH" "$BYPASS_TTL" <<'PYEOF'
import sys, os, json, shutil, importlib.util
core_path, settings_path, local_path, snap_dir, log_path, bypass_ttl = sys.argv[1:7]
...
# TTL bypass チェック
if core.is_bypass_active(os.path.expanduser("~/.claude"), ttl_seconds=int(bypass_ttl)):
    sys.exit(0)

# === PreToolUse: settings.json beforeスナップショット保存（全ツール）+ Bash チェインブロック ===
if hook_event == "PreToolUse":
    # before スナップショット保存（Gemini G2 critical採用・各ツール実行前にsettings.jsonを保存）
    # → Post で「このツール実行前 vs 実行後」を比較・ユーザー手動変更は誤認しない
    for target in [p for p in [settings_path, local_path] if os.path.exists(p)]:
        ...
        try:
            ...
            pass
    # Bash のみチェインブロック判定（Edit/Write 等は Bash チェイン概念なし）
    if tool == "Bash":
        cmd = d.get("tool_input", {}).get("command", "")
        if core.pre_detect_exfil_chain(cmd):
            core.write_log(log_path, "PRE_BLOCKED", "書込+送信チェイン事前ブロック", suspect_value=cmd[:60])
            print(json.dumps({"decision": "block", "reason": "guard-settings-write: settings.json 書込 + ネットワーク送信の同一チェインを検知（Pre補助・事前ブロック）"}, ensure_ascii=False), file=sys.stderr)
            sys.exit(2)
    sys.exit(0)
