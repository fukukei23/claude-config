...
- **⚠️ プロキシ操作時の警告ゲート（厳格・2026-07-03事故対策）**:
  ...
  - 実行前に**必ずふくけいへ警告し承認を取得**すること:「これからプロキシを再起動します・全セッションがN秒停止します・よろしいですか？」
...
報告・要約・完了通知・設計判断の提示・選択肢提示・リスク説明など「ふくけいが理解して判断・承認する必要がある場面」では、専門的な説明に**加えて**、素人でもわかる平易な解説を**必ず併記**すること。専門用語を羅列した「要約」は情報の羅列でしかなく、**理解できないまま承認・判断させる危険**がある。
...
タスク完了・トピック区切りで記録すべき内容が生じたら、**自発的・自動的に `ssot-record` スキルを発動**（ふくけいが「記録して」と言わなくても・`[SSOT-RECORD-TRIGGER]` 等のトリガー也不要）。
...
- **memory過信禁止・SSOT正典優先**: 記録前に自問→「能力・方針・構造か?」→`00_SYSTEM` / 「なぜそう決めたか理由あるか?」→`01_DECISIONS` / 「私的失敗の直し方か?」→memory。memoryは毎回読まれるが個人FB専用・永久保存版は00_SYSTEM正典へ。ふくけい指摘を待たず自ら判定（2026-07-24 e36d引継ぎ・Gemini+MiniMaxレビュー反映）。
...
- **作業着手前**に🟢行を確認。着手タスクが🟢（他セッション占有中）なら、ふくけいにsoft警告:
  > ⚠️「<タスク>」は <窓> が <時刻>〜進行中です。重複着手しますか？
- ブロックせずふくけい判断。new-session で占有解放（タスクはバックログに戻る）
- 詳細: `docs/superpowers/specs/2026-06-23-task-occupancy-board-design.md`

## スキルトリガー（厳格）
ふくけい発言がスキルのトリガーワード（各スキルファイル内「トリガーワード」欄）に合致する場合、必ず先に Skill ツールで該当スキルを発動してから対応せよ
