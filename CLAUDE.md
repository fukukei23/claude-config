...
## 外向き文面の実績数値は正典から（厳格・2026-08-15）
応募文・経歴書・ポートフォリオ・記事・ピッチなど**外向き文面に実績数値（プロジェクト件数・行数・テスト数等）を書く時は、必ずその場で `obsidian-ssot/40_CAREER/01_ドキュメント/ポートフォリオ数値マスター.md` の実測値・正典表記を確認して使う**。memory・記憶・過去文面の概数をそのまま書くことは禁止（2026-08-15 応募文に古い「20+プロジェクト」を混入した事故の再発防止・正典ルールはmemoryに頼らず本層1に置く）。
