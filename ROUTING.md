...
| 追加課金 | $5/月上限 | トークン超過時 |
| GLM API（MCP経由） | Z.AI従量 | コード生成等 |
| MiniMax API（MCP経由） | 従量 | 日常タスク・要約等 |

### CLI環境のコスト

| コンポーネント | コスト | 備考 |
|---|---|---|
| Z.AI API（GLM-5.1） | 従量 | ベースモデル |
| MiniMax API（フォールバック） | 従量 | 429/500系エラー時 |
| Sonnet（最終フォールバック） | 事前許可制 | APIキー設定が必要 |

### コスト削減の仕組み

1. **Desktop**: Sonnet固定だが、MCP経由でMiniMax/GLMに処理を委譲 → Sonnetのトークン消費を最小化
2. **CLI**: GLM-5.1（Z.AI）をベースにすることで、Anthropic APIコストを大幅削減
...
| バッジ | LLM | 利用シーン | コスト |
|---|---|---|---|
| 🟠 | MiniMax | 日常作業・分析・要約・翻訳・バッチ処理 | 最安 |
| 🟡 | GLM-5.1 | コード生成・MiniMaxが遅い/失敗した場合 | 中 |
...
```
MiniMax呼び出し → 失敗 → GLM呼び出し → 失敗 → ユーザー確認 → Sonnet使用
```
...
- `README.md` — リポジトリ概要
- `ARCHITECTURE.md` — 2環境アーキテクチャ図
- `TROUBLESHOOTING.md` — トラブルシューティング
- `fallback-config.json` — CLI版フォールバック設定
- `settings.example.json` — CLI設定テンプレート
- `docs/CONFIG-STRATEGY.md` — 設定戦略比較

### obsidian-ssot 参照

詳細な知識ベースは `obsidian-ssot/01_DECISIONS/claude-code/` を参照:
- `claude-code-llm-routing.md` — ルーティング設定詳細
- `claude-code-llm-badge-and-cost.md` — バッジ・コスト詳細
- `claude-code-architecture.md` — アーキテクチャ仕様
