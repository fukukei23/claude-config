...
SESSION="${1:-ssot}"
CLAUDE="/home/yn4416/.npm-global/bin/claude"
MONITOR_CMD='watch -n 5 "tmux ls && tmux list-panes -a"'

# セッション存在確認（なければ作成）
if ! tmux has-session -t "$SESSION" 2>/dev/null; then
  echo "セッション '$SESSION' を新規作成..."
  tmux new-session -s "$SESSION" -d -c /home/yn4416/vaults/SSOT
fi
...
echo "6ペイン作成中..."
for i in $(seq 5); do
  tmux split-window -t "$SESSION:0.0" -c /home/yn4416/vaults/SSOT 2>/dev/null
done
