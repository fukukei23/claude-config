...
name: repo-review
description: >
  任意のリポジトリ（アプリ/ライブラリ/運用スクリプト/コンテンツ等）の種別を判定し、
  種別に応じた評価軸（品質・テスト・依存、または検証ゲート等）で**実測ベース**評価して優先度付き改善案を出すスキル。
...
## Pre-flight: 浅い用途でないかの確認（1問・Phase 0 の前）
...
## Phase 2.5: 計測健全性チェック（🔴 新設・省略禁止・Phase 3 の前）
