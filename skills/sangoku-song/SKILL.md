---
name: sangoku-song
description: 三国志武将のHIPHOP楽曲を、武将選択から曲+歌詞+画像+動画まで質問ドリブンで制作するウィザード。ユーザーが「三国志」「sangoku」等と言った時、または /sangoku-song を呼んだ時にトリガー。
---

# 三国志HIPHOP制作ウィザード

## トリガーワード
「三国志」「sangoku」「/sangoku-song」
...
選択肢（武将カタログから代表）:
  ...
  ＋「その他（カタログ参照）」
```
> **💡 外部楽曲ベースの場合**: YouTube動画から世界観・構造を逆コンパイルしたい時は `reverse-engineer-song` スキルを使用（「YouTubeから分析して」等）。出力仕様書を本skillの Phase 0「その他」経由で持ち込み可能。

> **💡 汎用の作曲・作詞（テーマ不限）の場合**: `make-song` スキルを使用（「曲作って」「作曲して」等）。本スキルは三国志HIPHOP固有だが、作曲エンジン部分は `make-song` に共通化されている。必要に応じて `make-song` の参照（プロンプト技法・構造理論・発音問題等）を活用可。
