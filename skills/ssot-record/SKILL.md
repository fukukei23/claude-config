...
|---|---|---|
| **R: 層1ルール化を提案** | **踏む前に読んでいないと避けられない**性質、かつ**踏む前には検索キーワードを思いつけない**（＝事後の grep では手遅れ）。例: 「0件を根拠に結論を出すな」型 | **追記しない**。完了報告に「層1ルール化の候補」として1行提案するのみ。層1（`rules/_shared/`・毎回読込）の肥大化はコストが高いので**勝手に書かない**。ここだけはユーザー判断 |
| **H: ハブに1行追記** | 同じ `root_cause.category` かつ共通タグ**2つ以上**の過去記録が **2件以上**（今回で**通算3回目以上**） | `ハブマッピング.md` で決まるハブに1行追記。**3点以上は「パターン」**であり、個別記録を全部読まないと見えない |
| **N: 逆リンクのみ（既定）** | 上記いずれにも当たらない（**通算2回目まで**） | フェーズ3-1-α の逆リンクのみ。**2点は線を引けば辿れる**ので第3の文書は不要 |

### N へ降格するガード（H より優先）

- `ハブマッピング.md` に該当ハブが**無い** → **N**。新規ハブを作ってまで残さない
- 追記先候補が `01_DECISIONS/` 配下の**個別記録** → **N**（記録間の関係は逆リンクで表す）
- 既に同趣旨の1行がそのハブにある → **N**（重複させない・既存行の日付だけ更新してよい）

### 通算件数の数え方

まずフェーズ0.8の候補リスト（追加I/Oなし）で、今回と `root_cause.category` が一致し
`tags` が2つ以上重なるものを数える。候補は「過去7日 ∪ 直近5件」なので取りこぼしうるため、
**H の条件に当たりそうな時だけ**全期間を1回 grep して確定させる:

```bash
grep -rl "category: <今回のroot_cause.category>" ~/projects/obsidian-ssot/01_DECISIONS/ \
  | xargs grep -l "<共通タグ1>" | xargs grep -l "<共通タグ2>" | wc -l
```

> ⚠️ この件数を根拠にする前に**必ず中身を数行目視する**（`@rules/_shared/LLMサボりバイアス防止.md`
> 「検索結果を事実として使う前の確認」）。0件・N件をそのまま信じない。
...
📖 ガイド転記: なし（または「あり → <章名>」）
🔗 関連パターン: <N|H|R> — <理由1行>（通算<n>回目）  ← **related_patternが非nullなら省略不可**。
   N=逆リンクのみ / H=<ハブ名> に1行追記済み / R=層1ルール化の候補（未追記・ユーザー判断待ち）
