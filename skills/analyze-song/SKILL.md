...
- **ゴールドセット3組**（Lemon→ドライフラワー/Bohemian→ROCK/HUMBLE.→HIPHOP・spec 3.2推奨）: progression正規化（2026-06-24）で chord 軸はクリーン三和音ラベル化・スコア実効値化したが、n-gram Jaccard 指標では同/異ジャンルでスコア差なくジャンル判別不可。3組とも **xfail**（chord 指標見直しタスクで green 化予定）。詳細 `tests/golden_set.yaml` + spec 2.3
