...
> | **B. 中高音使用率**（新） | ウィスパー低音偏り | 50%以上 ✅・30〜50% ⚠️・30%未満 ❌ | ウィスパー低音・抑揚なし |
> | **C. 抑揚幅スコア**（新） | 平板メロディ | 5音以上 ✅・3〜5 ⚠️・3音未満 ❌ | 抑揚なし・平板メロディ |
>
...
- **3軸チェック**:
  ...
  - **軸B（中高音使用率）**: プロンプトで予防制御。生成後にイヤホンで聴いて「低音ウィスパーに偏る」と感じたら ❌。改善は声質プロファイル強化（`high range usage mandatory`、`belt as primary`、`A4+ dominant` を追加）
  - **軸C（抑揚幅）**: プロンプトで「5音以上の起伏」を明記。生成後に「単調」「平板」と感じたら ❌。改善は「メロディーに動きをつける」「サビで跳躍」を追加
- **チェックツール**: `python3 scripts/check_song_quality.py --bpm 98 歌詞.md`（軸Aのみ自動・軸B・Cは手動評価）
