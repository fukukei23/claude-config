...
- **「次のタスク」候補には `active-sessions.md` の🟢進行中タスク表に載っているタスクを含めないこと**（重複着手防止）。代わりに「前回占有タスク（継続可・参考）」欄に現セッションの占有タスクを記載すること
...
## 次のタスク（候補）
[**🟢進行中タスク表に載っていない（＝誰も占有していない）タスクのみ**を候補として列挙。バックログP0+P1から、他セッション占有中のタスクを除外したもの]
