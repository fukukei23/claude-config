...
9. ⑨選択肢ラベルの使い回し（往復中の対話で一度使ったA/B/C等のラベルに別の意味を与えて再利用→相手の「Bです」がどちらのBか判別不能になる。新たな選択肢を出す時はラベルを変えるか自然文で例示する。2026-08-19 深井神話第2報・log 20260819-03）
...
- **選択肢ラベルは対話シリーズで一貫させる**（⑨封じ）: 過去の往復でA/B/C等のラベルを使った場合、次の文面で同じラベルに別の意味を与えない。新しい選択肢はラベル無しの自然文例示（例: 「○○に絞りたい場合はその旨お知らせください」）が安全
