...
**監査モードの処理は「監査フロー（Step 1〜4）」セクションに従う。** 以降のフェーズ1〜5は監査モードでは使用しない。
...
#### 観点2: トリガー競合（誤発火リスク）
- 他スキルと同じトリガーワードが無いか（特に prefix 関係: 「曲作って」が make-song と sangoku-song で重複等）
...
合計点 > 0 のスキルのみ、フェーズ4（既存レポートフォーマット）と同様に 🔴/🟡/🔵 別で列挙:
