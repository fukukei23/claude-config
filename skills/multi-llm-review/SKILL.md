...
> **📝 コードレビュー時の入力素材（2026-09-06追加・網羅盲点対策）**: diff に加えて
> **対象ファイル全体+既存テスト一式**をプロンプトに含めること。diffだけだと
> 「対象機能の全挙動」が見えず、未テスト領域の列挙（観点パック参照）が機能しない
> （2026-09-05実例: 3機レビュー計39指摘がいずれもdiff外の未テスト領域〔死亡時表示・
> WARNING行・未テスト分岐〕を列挙できず・人間の「テストは充分？」1問で3穴発見）。
> 入力肥大時は対象ファイルの挙動一覧（関数一覧+分岐）への圧縮+Gemini圧縮リトリアを併用。
...
| コード | 正確性 / 可読性 / 性能 / セキュリティ / テスタビリティ / 既存規約整合 / **未テスト領域の列挙**（対象機能の全挙動を黒箱列挙→テスト存在照合→欠落を**上位3件に限定**して報告・2026-09-06追加） |
...
9. **構造変化時の再レビュー確認（2026-09-01・L66）**: 改訂で候補追加・ゲート新設・判断の反転等の**案の構造変化**が起きた場合、ホストは「収束した」と自己判定せず、**完了報告に「もう一度各LLMでレビューしますか？」を必ず添える**。文言修正・表記統一レベルの改訂なら添付不要。発動根拠: 2026-09-01 ミューテーション導入判断で「収束判定（3回目は収益薄）」の後、3ラウンド目が26件（critical約7件）を産み判定が実測で否定された（正典: `00_SYSTEM/共通ルール/自律開発ループ.md`「判断収束ループの発火条件」節・同2026-09-01追記と同一ルール）
